...
void MainThread::search() {
  ...
  if (bestThread != this) {
      if (this->searchType == Search::STRUCTURED) {
        // add to searchInfo 
        sync_cout << UCI::pv(bestThread->rootPos, bestThread->completedDepth) << sync_endl;
        this->searchInfo.push_back(UCI::structuredPv(bestThread->rootPos, bestThread->completedDepth));
      }
      sync_cout << UCI::pv(bestThread->rootPos, bestThread->completedDepth) << sync_endl;
  }

  if (this->searchType == Search::STRUCTURED) {
    // add bestMove to search output json object
    sync_cout << "bestmove " << UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());
    searchOutput["bestMove"] = UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());
  } else {
    sync_cout << "bestmove " << UCI::move(bestThread->rootMoves[0].pv[0], rootPos.is_chess960());
  }
  ...
  if (bestThread->rootMoves[0].pv.size() > 1 || bestThread->rootMoves[0].extract_ponder_from_tt(rootPos)){
      if (this->searchType == Search::STRUCTURED) {
        // add ponder to search output json object
        std::cout << " ponder " << UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());
        searchOutput["ponder"] = UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());
      } else {
        std::cout << " ponder " << UCI::move(bestThread->rootMoves[0].pv[1], rootPos.is_chess960());
      }
  }
  ...
  if (this->searchType == Search::STRUCTURED) {
    // add info array from search to search output json object
    searchOutput["searchInfo"] = this->searchInfo;
    std::cout << sync_endl << searchOutput.dump() << sync_endl;
  }
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          while (true)
          {
              ...
              if (   mainThread
                  ...
                  std::lock_guard<std::mutex> lock(mainThread->mutex);
                  // add PV info to searchInfo 
                  mainThread->searchInfo.push_back(UCI::structuredPv(rootPos, rootDepth));
...
nlohmann::json UCI::structuredPv(const Position& pos, Depth depth) {
    ...
    for (size_t i = 0; i < multiPV; ++i) {
        ...
        nlohmann::json pvInfo;
        pvInfo["rootFen"] = pos.fen();
        pvInfo["depth"] = d;
        pvInfo["seldepth"] = rootMoves[i].selDepth;
        pvInfo["multipv"] = i + 1;
        pvInfo["enginePOVScore"] = UCI::value(v);
        pvInfo["enginePOVEval"] = Eval::format_cp_for_structured_output(v);

        if (Options["UCI_ShowWDL"])
            pvInfo["wdl"] = UCI::structuredWdl(v, pos.game_ply());

        if (i == pvIdx && !tb && updated) // tablebase- and previous-scores are exact
            pvInfo["bound"] = rootMoves[i].scoreLowerbound ? "lowerbound" : (rootMoves[i].scoreUpperbound ? "upperbound" : "");

        pvInfo["nodes"] = nodesSearched;
        pvInfo["nps"] = nodesSearched * 1000 / elapsed;
        pvInfo["hashfull"] = TT.hashfull();
        pvInfo["tbhits"] = tbHits;
        pvInfo["time"] = elapsed;

        nlohmann::json pvMoves;
        for (Move m : rootMoves[i].pv)
            pvMoves.push_back(UCI::move(m, pos.is_chess960()));

        pvInfo["pv"] = pvMoves;

        jsonOutput.push_back(pvInfo);
    }
